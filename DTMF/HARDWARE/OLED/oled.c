...
//y:0~63
//mode:0,反白显示;1,正常显示				 
void OLED_ShowCHI(u8 x,u8 y,u8 chr,u8 mode)
{      			    
	u8 temp,t,t1;
	u8 y0=y; 
    for(t=0;t<24;t++)
    {   
		temp=Chinese_1212[chr][t]; 	 	//调用1206字体
        for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)OLED_DrawPoint(x,y,mode);
			else OLED_DrawPoint(x,y,!mode);
...
void OLED_ShowBMP(u8 x, u8 y, u8 px, u8 py, u8 index, u8 mode)
{
    u16 temp,t1,t;
    u8 y0=y;
    u16 csize=(py/8+((py%8)?1:0))*(px);
 
    for(t=0;t<csize;t++)
    {
        temp=Logo_WYU[index][t];                             
        for(t1=0;t1<8;t1++)
        {
            if(temp&0x80)OLED_DrawPoint(x,y,mode);
            else OLED_DrawPoint(x,y,!mode);
...
{ 	
 ...
 #else
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_2;				 //PC0,1 OUT推挽输出
 	GPIO_Init(GPIOC, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOC,GPIO_Pin_0|GPIO_Pin_2);						 //PC0,1 OUT  输出高
